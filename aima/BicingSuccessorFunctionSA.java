		...
		do {
			...
			case 0:
				...
				if (estCiudad.transportes.size() < Ciudad.getNumFurgonetas()) {
					...
					if (!estCiudad.estacionesOcupadas.contains(origen)) {
						...
						// Si la parada uno es diferente de la origen, seguimos
						if (paradaUno != origen) {
							...
							if (bcParadaUno < bcOrigen) {
								...
								// Si la parada dos coincide con alguna, dejamos
								// todas las bicis en
							...
							// Si no nos sobran bicis, solo tendremos una parada
							else {
								Ciudad nuevaCiudad = new Ciudad(estCiudad);
